...
# TODO: make the logging more robust so the reaction get there own subdirs for the date and hour so there is not an appending to the same files for the simualtion 
...
def run_md_simulation_with_reaction_capture(
    ...
    # Adding path things for the current data time to the bonding log
    now = datetime.now()
    datetime_string = now.strftime("%Y-%m-%d %H:%M:%S")
    bonding_log_path = os.path.join(output_dir, datetime_string + "_bonding.log")
    ...
    try:
        ...
        print(f"\nWriting the left over reactions that were captured but didnt fill the frame buffer.")

        current_frame = traj_manager.frame_counter
        abs_time = current_frame * traj_manager.frame_interval * (traj_manager.timestep/units.fs)
        traj_manager.frame_counter += 1

        frame_data = (atoms.copy(), abs_time)
        traj_manager.pre_reaction_buffer.append(frame_data)
...
if __name__ == "__main__":
    '''
    # Setup to change fs and temp with argparse
    
    parser = argparse.ArgumentParser(description="Run HClWaterMD with reaction capture.")
    input_file: str = "./examples/HClWater.xyz",
    output_dir: str = "output",
    output_file: str = None,
    cell_size: float = 25.25,
    ...
    friction: float = 0.01 / units.fs,
    log_interval_fs: float = 10.0,              # Time between log entries (fs)
    total_time: float = 10000.0, # 1000.0,                # Total simulation time in fs
    md_traj_interval_fs: float = 500.0,        # Time between main trajectory frames (fs)
    bond_check_interval_fs: float = 10.0,       # Time between bond checks (fs)
    ema_alpha: float = 0.1,                     # Smoothing factor for the EMA of the bonding graph
    reaction_threshold: float = 0.6,            # Threshold for detecting reactions
    ...
    parser.add_argument(
        "input_file", type=str, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "output_dir", type=str, help="Name of the directory to save the output to Defaults to output."
    )
    parser.add_argument(
        "output_file", type=str, help="Name of the output file defaults to None"
    )
    parser.add_argument(
        "cell_size", type=float, help="The boundary conditions of the simulation defaults to 25.25"
    )
    parser.add_argument(
        "temperature_K", type=float, help="Initial temperature of the simulation in kelvin, defaults to 300."
    )
    parser.add_argument(
        "timestep", type=float, help="Dt of the simulation in fs defaults to .5 * units.fs"
    )
    parser.add_argument(
        "friction", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "log_interval_fs", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "total_time", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "md_traj_interval_fs", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "bond_check_interval_fs", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "ema_alpha", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "reaction_threshold", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "vdw_multiplier", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "reaction_time_window", type=float, help="Name of the starting simulation file to simulate."
    )
    parser.add_argument(
        "reaction_traj_interval_fs", type=float, help="Name of the starting simulation file to simulate."
    )
